...
export default function CustomersDashboard() {
  ...
  return (
    <div className="p-8 space-y-8 bg-[#FBFBF2]/60 min-h-screen">
      {/* Page Header */}
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-4xl font-serif font-black text-[#1A4D2E]">Customer Directory</h1>
          <p className="text-[#5F6F65] font-semibold mt-1">Monitor profiles, loyalty metrics, and historical spending data.</p>
        </div>
        <button
          onClick={() => { fetchStats(); fetchCustomersList(currentPage, searchQuery); }}
          className="p-3 bg-white hover:bg-[#E8F5E9] border border-[#EFE8D8] text-[#1A4D2E] rounded-xl transition-all shadow-sm flex items-center gap-2 font-bold text-sm"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-[#EFE8D8] shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-xs uppercase font-black tracking-wider text-gray-400">Total Customers</p>
            <h3 className="text-3xl font-black text-[#1A4D2E]">{stats.totalCustomers}</h3>
            <p className="text-[10px] text-gray-400 font-bold">Registered profiles in database</p>
          </div>
          <div className="h-12 w-12 rounded-2xl bg-[#E8F5E9] text-[#1A4D2E] flex items-center justify-center">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-[#EFE8D8] shadow-sm overflow-hidden flex flex-col">
          {/* List Toolbar */}
          <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row gap-4 items-center justify-between bg-white/40">
            <h3 className="text-lg font-black text-[#1A4D2E] tracking-tight">Customer Database</h3>
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 rounded-full border border-gray-200 focus:border-[#1A4D2E] focus:outline-none text-xs font-semibold"
              />
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/70 border-b border-gray-100 text-[10px] font-black text-[#1A4D2E] uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-gray-50 text-xs font-semibold text-gray-700">
                  {customers.map((c) => (
                    <tr key={c.id} className="hover:bg-[#FBFBF2]/30 transition-colors">
                      ...
                      <td className="py-4 px-6 text-right text-sm font-black text-[#1A4D2E]">
                      ...
                      <td className="py-4 px-6 text-center">
                        <button
                          ...
                          className="p-2 hover:bg-[#E8F5E9] text-[#1A4D2E] rounded-lg transition-colors border border-transparent hover:border-[#1A4D2E]/20"
                        >
        ...
        <div className="bg-white rounded-[2.5rem] border border-[#EFE8D8] shadow-sm overflow-hidden flex flex-col p-6 min-h-[500px]">
          {selectedCustomerId ? (
            profileLoading ? (
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-xl font-black text-[#1A4D2E] tracking-tight">{customerProfile.name}</h3>
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 bg-[#E8F5E9] hover:bg-[#4ADE80]/20 text-[#1A4D2E] rounded-xl text-xs font-black transition-colors flex items-center gap-1 border border-[#1A4D2E]/20"
                  >
                ...
                {isEditing ? (
                  ...
                  <form onSubmit={handleEditSubmit} className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-black text-[#1A4D2E] tracking-wider ml-1">Name</label>
                      <input
                        required
                        type="text"
                        value={editFormData.name}
                        onChange={(e) => setEditFormData({ ...editFormData, name: e.target.value })}
                        className="w-full p-2.5 rounded-xl border border-gray-200 focus:outline-none focus:border-[#1A4D2E] text-xs font-semibold text-gray-800"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-black text-[#1A4D2E] tracking-wider ml-1">Phone Number</label>
                      <input
                        required
                        type="text"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-black text-[#1A4D2E] tracking-wider ml-1">Email</label>
                      <input
                        type="email"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-black text-[#1A4D2E] tracking-wider ml-1">Birthday</label>
                      <input
                        type="date"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-black text-[#1A4D2E] tracking-wider ml-1">Loyalty Points</label>
                      <input
                        type="number"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-black text-[#1A4D2E] tracking-wider ml-1">Membership Tier</label>
                      <select
                        value={editFormData.membershipLevel}
                        onChange={(e) => setEditFormData({ ...editFormData, membershipLevel: e.target.value })}
                        className="w-full p-2.5 bg-white rounded-xl border border-gray-200 focus:outline-none focus:border-[#1A4D2E] text-xs font-semibold text-gray-800"
                      >
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <button
                        type="submit"
                        className="flex-1 py-2.5 bg-[#1A4D2E] hover:bg-[#143d24] text-white font-bold rounded-xl text-xs transition-colors flex items-center justify-center gap-1 shadow-sm"
                      >
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2 bg-[#FBFBF2] p-4 rounded-2xl border border-gray-100 text-xs font-semibold text-gray-700">
                      ...
                      {customerProfile.email && (
                        <div className="flex items-center gap-2">
                          <Mail className="h-4 w-4 text-[#5F6F65]" />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-[#E8F5E9]/30 p-4 rounded-2xl border border-emerald-50 text-center space-y-1">
                        <span className="text-[10px] font-black uppercase text-[#1A4D2E] tracking-wider">Tier</span>
                    ...
                    <div className="space-y-3 bg-[#FBFBF2] p-4 rounded-2xl border border-gray-100">
                      <div className="flex justify-between border-b border-gray-100 pb-2 text-xs">
                        ...
                        <span className="font-extrabold text-[#1A4D2E]">Top 3</span>
                      </div>
                    ...
                    <div className="space-y-3">
                      <h4 className="text-xs font-black text-[#1A4D2E] uppercase tracking-wider ml-1">Complete Order History</h4>
                      {customerProfile.orders && customerProfile.orders.length > 0 ? (
                        <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                          {customerProfile.orders.map((ord) => (
                            <div key={ord.id} className="p-3 bg-white border border-gray-100 rounded-xl hover:border-[#1A4D2E]/20 transition-all flex justify-between items-center text-xs">
                              <div>
                              ...
                              <div className="text-right">
                                <p className="font-black text-[#1A4D2E]">₹{Number(ord.totalAmount).toFixed(2)}</p>
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center opacity-60">
              <div className="h-16 w-16 bg-[#FBFBF2] border border-[#EFE8D8] rounded-full flex items-center justify-center mb-3">
                <User className="h-7 w-7 text-gray-400" />
              </div>
              <h4 className="font-bold text-[#1A4D2E] text-sm">Select a Customer</h4>
              <p className="text-xs text-[#5F6F65] max-w-[200px] mt-1">Click the eye action button on a profile row to view details, stats, loyalty levels, and order histories.</p>
            </div>
          )}
        </div>
      </div>
